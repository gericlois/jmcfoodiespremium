...
$sql = "SELECT * FROM communication_log WHERE module = 'wellness'";
...
<div class="container-fluid py-4">
  <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-4">
    <div class="d-flex flex-wrap gap-2">
      <a href="<?= BASE_URL ?>/admin/communication_log.php" class="filter-pill <?= $channel_filter === '' ? 'active' : '' ?>">All</a>
      <a href="<?= BASE_URL ?>/admin/communication_log.php?channel=sms" class="filter-pill <?= $channel_filter === 'sms' ? 'active' : '' ?>">SMS</a>
      <a href="<?= BASE_URL ?>/admin/communication_log.php?channel=email" class="filter-pill <?= $channel_filter === 'email' ? 'active' : '' ?>">Email</a>
    </div>
    <button type="button" class="btn-outline-theme no-print" onclick="window.print()"><i class="fas fa-print"></i>Print</button>
  </div>

  <p class="text-muted small mb-3">Showing the most recent 300 Wellness SMS/email sends — automatic triggers and admin-initiated messages alike.</p>
