    ...
    if ($action === 'approve') {
        ...
        if ($approved) {
            ...
            if ($member) {
                ...
                // (basics + last name) so the notifications below can show
                // the member an actual working password, rather than just
                // pointing back at whatever they typed at signup.
                $new_password = basics_default_password($member['full_name']);
                $hash = password_hash($new_password, PASSWORD_DEFAULT);
                $stmt = $conn->prepare("UPDATE basics_users SET password_hash = ?, must_change_password = 1 WHERE id = ?");
                $stmt->bind_param('si', $hash, $member['user_id']);
                $stmt->execute();
                $stmt->close();

                basics_notify($conn, $member, "Hi {$member['full_name']}, your JMC Foodies Basics membership has been APPROVED! Weekly credit limit: " . format_price($weekly_limit) . ". Log in with username \"{$member['username']}\" and password \"{$new_password}\" - JMC Foodies Basics");
                send_basics_account_approved_email($member['email'], $member['full_name'], $member['username'], $weekly_limit, $new_password);
            }
        }
    } elseif ($action === 'deny') {
