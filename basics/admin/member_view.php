...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_credit') {
        ...
        $stmt = $conn->prepare("SELECT bm.user_id, u.full_name FROM basics_members bm JOIN basics_users u ON u.id = bm.user_id WHERE bm.id = ?");
        $stmt->bind_param('i', $id);
        $stmt->execute();
    ...
    <div class="col-12 col-md-6">
      <div class="panel-card mb-4">
        ...
        <form method="post" class="d-inline">
          ...
          <button type="submit" class="btn-chip btn-chip-outline" onclick="return confirm('Reset this member\'s password to the default (basics + last name)? They will be required to set a new one on next login.');">Reset to Default Password</button>
        </form>
      </div>
